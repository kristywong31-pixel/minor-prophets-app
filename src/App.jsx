...
// ------------------------------------------------------------------
// 重要：請確保你的 src/supabaseClient.js 檔案內容是：
// export const supabase = createClient(supabaseUrl, supabaseAnonKey);
// 如果是用 export default，請自行拿掉下面這一行的花括號 { }
// ------------------------------------------------------------------
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 如果有 session，直接讀取 app_users
      // 注意：這時候因為已經有 session，所以 RLS 不會擋
      ...
      if (error || !profile) {
        // 有 session 但沒 profile，可能是資料庫被清空，強制登出
  ...
  const handleAuth = async ({ mode, payload }) => {
    ...
    try {
      if (mode === "register") {
        ...
        // 2. 寫入 app_users (因為剛註冊成功，現在有權限寫入自己的資料)
        ...
        if (profileErr) {
          ...
          // 雖然 Auth 成功但 Profile 失敗，這是一個邊緣情況，通常是因為 RLS 設定錯誤
          throw new Error("建立個人資料失敗，請聯繫管理員。");
        }
        ...
        // Login 模式
        // 1. 直接嘗試登入 Supabase Auth (不需要先查資料庫，避開 RLS)
        ...
        if (signInErr) {
          ...
          throw new Error("登入失敗：名字或密碼錯誤。");
        }

        // 2. 登入成功後，再去讀取 Profile
        const { data: profile, error: profileErr } = await supabase
          .from("app_users")
          .select("id, name, note, avatar_url, avatar_color")
        ...
        if (profileErr || !profile) {
          throw new Error("找不到使用者資料，請重新註冊。");
        }
